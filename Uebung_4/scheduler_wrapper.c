...
#ifndef CONFIG_PROCS
#define CONFIG_PROCS \
{ { .atime = 0,  .rtime = 3, }, { .atime = 2,  .rtime = 7, }, { .atime = 4,  .rtime = 1, }, { .atime = 6,  .rtime = 5, } }
#endif
...
__attribute__((weak, alias ("empty")))
void rr();
__attribute__((weak, alias ("empty")))
void fcfs();
__attribute__((weak, alias ("empty")))
...
static bool run_and_print_queue(struct process *const HEAD)
{
	...
	for (struct process *current = HEAD->next;
	     current != HEAD;
		...
		switch (current->state) {
        	case PS_RUNNING:
                	nrunning++;
                	assert(current->cycles_done + current->cycles_todo <= processes[current->pid].rtime);
			current->cycles_done++;
		...
		case PS_DEAD:
        		assert(current->cycles_todo == 0);
        		assert(current->cycles_done == processes[current->pid].rtime);
        		/* nothing */
        		break;
		}

		printf("%u (R: %u/%u, W: %u): %8s\t",
		       current->pid,
		       current->cycles_done,
...
int main(void)
{
	...
	for (size_t ai = 0; ai < NUM_ALGOS; ai++) {
		...
		/*
                 ...
                 */
                assert(HEAD->next && HEAD->next->next && HEAD->next->next->prev);
